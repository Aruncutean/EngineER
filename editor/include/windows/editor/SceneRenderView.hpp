...
class SceneComponent : public component::IComponent {
public:
    SceneComponent(const std::string &label)
        : label(label) {
...
class SceneRenderView : public View {
public:
    SceneRenderView() : View("SceneRenderView") {
        noCollapse = true;
        alwaysAutoResize = true;

        auto sceneComponent = std::make_shared<SceneComponent>("Scene");
